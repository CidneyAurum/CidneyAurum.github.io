...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  listEl.addEventListener("click", () => {
    sessionStorage.setItem("blog_restore_scroll", String(window.scrollY));
  });

  const saveState = () => sessionStorage.setItem("blog_state", JSON.stringify({ keyword, activeTag, view, scrollY: window.scrollY }));
  if (searchInput) searchInput.addEventListener("input", () => { keyword = searchInput.value; render(); saveState(); });
  if (filterBar && allTags.length) {
    filterBar.innerHTML =
      '<span class="chip on" data-tag="">全部档案</span>' +
      allTags.map((t) => `<span class="chip" data-tag="${t}"># ${t}</span>`).join("");
  ...
  render();
  const rs = sessionStorage.getItem("blog_restore_scroll");
  if (rs) { sessionStorage.removeItem("blog_restore_scroll"); setTimeout(() => window.scrollTo({ top: +rs, behavior: "instant" }), 200); }
